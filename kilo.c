...
// returns true if a separator character
...
char *editorPrompt(char *prompt, void (*callback)(char *, int)) {
    ...
    while (1) {
        ...
        // \x1b: ESC key
        else if (c == '\x1b') { // cancel the input prompt.
...
void editorSave() {
    ...
    free(buf);
    editorSetStatusMessage("Can't save! I/O editor: %s", strerror(errno));
}
...
void editorFindCallback(char *query, int key) {
    ...
    // current: the index of the current row we are searching.
